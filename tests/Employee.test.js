...
describe('Employee', () => {
    ...
    describe('Initialization', () => {
        ...
        it('should throw an error if provided zero arguments', () => {
            ...
            expect(addEmployee).toThrowError();
        });

        it("should throw an error if not provided an id", () => {
            const addEmployee = () => new Employee('Lindsey', 'lindsey@test.com');
            // Define the error message that is expected to be thrown
            const err = new Error("Expected parameter 'id' to be a non-negative number");
            // Verify that the correct error was thrown when the callback is executed
            expect(addEmployee).toThrowError(err);
        });

        it("should throw an error if 'name' is not a string", () => {
            const addEmployee = () => new Employee(Lindsey, 25, 'lindsey@test.com');
        ...
        it("should throw an error if 'email' does not mirror the characters used in an email address", () => {
            const addEmployee = () => new Employee('Lindsey', 25, 'lindseytest.com');
